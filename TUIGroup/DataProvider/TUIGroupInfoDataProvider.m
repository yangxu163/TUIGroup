...
- (void)setupData
{
    ...
    if (self.groupInfo) {
        ...
        TUICommonTextCellData *changeSuggestItem = [[TUICommonTextCellData alloc] init];
        changeSuggestItem.key = TUIKitLocalizableString(ProfileSuggest);
        changeSuggestItem.cselector = @selector(didSelectOnSuggest:);
        changeSuggestItem.showAccessory = YES;
        [dataList addObject:@[changeSuggestItem]];
...
- (void)didSelectOnSuggest:(TUICommonTextCell *)cell {
    if (self.delegate && [self.delegate respondsToSelector:@selector(didSelectOnSuggest:)]) {
        [self.delegate didSelectOnSuggest:cell];
    }
}
